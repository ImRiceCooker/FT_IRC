...
{
	if (port.length() > 5 || !isAllNumber(port) || (_port = atoi(port.c_str())) > 65535)
...
void Server::prepare(void)
{
	...
	if ((_socket = socket(PF_INET, SOCK_STREAM, 0)) == -1 || setsockopt(_socket, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) == -1 || bind(_socket, (struct sockaddr *)&sockaddr, sizeof(sockaddr)) == -1 || listen(_socket, LISTEN_QUEUE_SIZE) == -1)
	{
		close(_socket);
		throw InitServerException();
	}
